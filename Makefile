...
ifdef CI
	...
	$(TF_ENV_VARS) terraform init -backend-config="bucket=$(PREFIX)-infra" -backend-config="dynamodb_table=$(PREFIX)-lock-infra" -backend-config="key=$(S3_PATH)/terraform.tfstate"
